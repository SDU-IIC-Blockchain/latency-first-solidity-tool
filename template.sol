...
contract __Template {
    ...
    function __tx_online_NAME(address[] memory addr, __tx_arg_NAME memory arg) public {
    ...
    function __tx_proof_NAME(address[] memory addr, __tx_arg_NAME memory arg) public {
        __state_def_NAME[] memory currentStates = new __state_def_NAME[](addr.length);
        ...
        for (uint i = 0; i < addr.length; i++) {
            bytes32 stateHash = __state_hash_NAME(newStates[i], arg);
            require(stateHash == __state_comp_dict_NAME[addr[i]].PendingStates[0], "commitment mismatch");
        }
    ...
    function __tx_offline_NAME(address[] memory addr, __state_def_NAME[] memory states, __tx_arg_NAME memory arg) public pure
    returns (__state_def_NAME[] memory newStates, bytes32[] memory newStateHashes){
        newStates = __tx_do_NAME(addr, states, arg);
        newStateHashes = new bytes32[](addr.length);
        for (uint i = 0; i < addr.length; i++) {
            newStateHashes[i] = __state_hash_NAME(newStates[i], arg);
        }
        return (newStates, newStateHashes);
    }

    function __tx_commit_NAME(address[] memory addr, bytes32[] memory hash) public {
        for (uint i = 0; i < addr.length; i++) {
            __state_comp_dict_NAME[addr[i]].PendingStates.push(hash[i]);
        }
    }
